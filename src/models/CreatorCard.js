...
const creatorCardSchema = new mongoose.Schema(
  {
    ...
    access_code: {
      ...
      validate: {
        ...
        message:
          "access_code must be exactly 6 alphanumeric characters for private cards, or null for public cards",
      },
    },
    deleted: {
      type: Number,
...
creatorCardSchema.pre("save", function () {
  // Ensure updated timestamp is set
  this.updated = Date.now();

  // Validate access_code based on access_type
  if (this.access_type === "public" && this.access_code) {
      (new Error("access_code can only be set on private cards"));
  } else if (this.access_type === "private" && !this.access_code) {
    (new Error("access_code is required when access_type is private"));
  } else if (
    this.access_type === "private" &&
    !/^[A-Za-z0-9]{6}$/.test(this.access_code)
  ) {
    (new Error("access_code must be exactly 6 alphanumeric characters"));
